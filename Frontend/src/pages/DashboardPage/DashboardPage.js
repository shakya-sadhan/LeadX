...
import { CampaignCreationView } from '../../components/dashboard/CampaignCreationView';
import { OutreachView } from '../../components/dashboard/OutreachView';
import { EmailSequenceCreationView } from '../../components/dashboard/EmailSequenceCreationView';
import { StepSequenceBuilder } from '../../components/dashboard/StepSequenceBuilder';
import { LeadOutreachView } from '../../components/dashboard/LeadOutreachView';
...
const DashboardPage = () => {
  ...
  const [systemActivities, setSystemActivities] = useState(sampleActivities);
  const [currentUser, setCurrentUser] = useState(sampleUsers[0]); // Default to admin user
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [selectedLeadForOutreach, setSelectedLeadForOutreach] = useState(null);
  const [selectedCampaign, setSelectedCampaign] = useState(null);
  const [preselectedLeadsForSequence, setPreselectedLeadsForSequence] = useState([]);
  
  // New state for managing categories
  const [leadCategories, setLeadCategories] = useState(['High Value', 'Medium Value', 'Low Value', 'Prospects', 'Hot Leads']);
  const [campaignCategories, setCampaignCategories] = useState(['Q4 2024', 'Partnerships', 'Product Launch', 'Customer Acquisition']);

  // New state for staged leads (leads generated but not yet saved to database)
  const [stagedLeads, setStagedLeads] = useState([]);

  const toggleLeadStar = (leadId) => {
    setLeads(prevLeads => 
      prevLeads.map(lead => 
        lead.id === leadId 
  ...
  const addLeads = (newLeads, chatTitle) => {
  ...
  const stageLeads = (newLeads, chatTitle) => {
    const defaultChatTitle = chatTitle || `Chat Session ${new Date().toLocaleDateString()}`;
    const leadsWithIds = newLeads.map(lead => ({
      ...lead,
      id: Math.random().toString(36).substr(2, 9),
  ...
  const editLeadCategory = (oldName, newName) => {
  ...
  const createCampaignLegacy = (name, description) => {
  ...
  const createOutreachSequence = (sequence) => {
    const newSequence = {
      ...sequence,
      id: Math.random().toString(36).substr(2, 9),
      createdAt: new Date()
  ...
  const updateOutreachSequence = (sequenceId, updates) => {
    setOutreachSequences(prev => prev.map(seq => 
      seq.id === sequenceId ? { ...seq, ...updates } : seq
    ));
  };

  const deleteOutreachSequence = (sequenceId) => {
  ...
  const startLeadOutreach = (lead) => {
    setSelectedLeadForOutreach(lead);
    setCurrentView('lead-outreach');
  };

  const createLeadOutreach = (emails, name, description) => {
  ...
  const startSequenceCreation = (preselectedLeads = []) => {
    setPreselectedLeadsForSequence(preselectedLeads);
    setCurrentView('sequence-builder');
  };

  const startSequenceCreationLegacy = (preselectedLeads = []) => {
    setPreselectedLeadsForSequence(preselectedLeads);
  ...
  const updateUserStats = (userId, statType, increment = 1) => {
    setUsers(prev => prev.map(user => 
      user.id === userId 
        ? { 
            ...user, 
  ...
  return (
    <div className="min-h-screen bg-background flex w-full">
      ...
      <div className="flex-1 flex flex-col w-full min-w-0">
        ...
        <main className="flex-1 overflow-hidden dashboard-main w-full">
